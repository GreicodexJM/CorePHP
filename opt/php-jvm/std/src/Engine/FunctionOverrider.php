...
namespace std\Engine;

use std\Security\Exceptions\EncodingException;
use std\Security\Exceptions\SecurityException;
use std\Security\Safe\FileReadException;
use std\Security\Safe\FileWriteException;
use std\Security\Safe\JsonDecodeException;
use std\Security\Safe\JsonEncodeException;
use std\Security\Safe\RegexException;
 ...
 *
 * @see /opt/php-jvm/bootstrap.php — calls FunctionOverrider::install() at boot
 */
final class FunctionOverrider
{
    ...
    private static function overrideJsonDecode(): void
    {
        ...
        runkit7_function_redefine(
            ...
            try {
                ...
                throw new \std\Security\Safe\JsonDecodeException(
    ...
    private static function overrideJsonEncode(): void
    {
        ...
        runkit7_function_redefine(
            ...
            try {
                ...
                if ($result === false) {
                    throw new \std\Security\Safe\JsonEncodeException(
    ...
    private static function overrideFileGetContents(): void
    {
        ...
        runkit7_function_redefine(
            ...
            if (!file_exists($filename)) {
                throw new \std\Security\Safe\FileReadException(
    ...
    private static function overrideFilePutContents(): void
    {
        ...
        runkit7_function_redefine(
            ...
            if ($result === false) {
                throw new \std\Security\Safe\FileWriteException(
    ...
    private static function overrideIntval(): void
    {
        ...
        runkit7_function_redefine(
            ...
            throw new \std\Security\Safe\TypeCoercionException(
    ...
    private static function overridePregMatch(): void
    {
        ...
        runkit7_function_redefine(
            ...
            if ($result === false) {
                throw new \std\Security\Safe\RegexException(
    ...
    private static function overrideCurlExec(): void
    {
        ...
        runkit7_function_redefine(
            ...
            if ($result === false) {
                ...
                throw new \std\Net\Http\HttpException(
    ...
    private static function overrideBase64Decode(): void
    {
        ...
        runkit7_function_redefine(
            ...
            if ($result === false) {
                throw new \std\Security\Exceptions\EncodingException(
    ...
    private static function overrideUnserializeGuard(): void
    {
        ...
        try {
            ...
            runkit7_function_redefine(
                ...
                '
                throw new \std\Security\Exceptions\SecurityException(
                    "unserialize() is permanently disabled in CorePHP (PHP-JVM). "
