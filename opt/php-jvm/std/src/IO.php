...
/**
 * std\IO — Safe I/O facade for file and HTTP operations.
 *
 ...
 *   // File operations
 *   $content = std\IO::read('config.json');         // FileReadException on failure
 *   $bytes   = std\IO::write('/tmp/out.txt', $data); // FileWriteException on failure
 *   $data    = std\IO::json('config.json');          // Reads + decodes JSON
 *
 *   // HTTP operations
 *   $response = std\IO::http()->get('https://api.example.com/users');
 *   $users    = std\IO::get('https://api.example.com/users')->json(true);
 *   $created  = std\IO::post('https://api.example.com/users', ['name'=>'Alice']);
 */
final class IO
{
    /**
